...
# - For child categories, we upsert by first ensuring parent exists, then using a stable parent_id variable
#   (avoids MySQL error 1093: target table for update in FROM clause).
...
query_scalar () {
  local sql="$1"
  # Returns the first column of the first row (no header)
  eval "${MYSQL_CMD} --protocol=TCP -N -s -e \"${sql}\""
}
...
# ---- categories ----
# NOTE: MySQL UNIQUE indexes allow multiple NULLs, so UNIQUE(parent_id, name_th) does not prevent
# duplicates for top-level categories where parent_id IS NULL. To make seeding idempotent, we:
# - Add generated column parent_id_key = COALESCE(parent_id, 0)
# - Enforce UNIQUE(parent_id_key, name_th)
#
# Also: self-referential FK creation can fail in some environments during migrations/alter operations
# (ERROR 1215). Uniqueness is the critical requirement for the seed; FK is optional.
run_sql "CREATE TABLE IF NOT EXISTS categories (id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, name_th VARCHAR(255) NOT NULL, name_en VARCHAR(255) NULL, type ENUM('income','expense','cogs','other') NOT NULL DEFAULT 'expense', parent_id BIGINT UNSIGNED NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, parent_id_key BIGINT UNSIGNED GENERATED ALWAYS AS (COALESCE(parent_id,0)) STORED, PRIMARY KEY (id), UNIQUE KEY uniq_categories_parent_name_th (parent_id, name_th), UNIQUE KEY uniq_categories_parentkey_name_th (parent_id_key, name_th), KEY idx_categories_type (type), KEY idx_categories_parent (parent_id) ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;"
...
# Resolve parent IDs (safe even if already present)
REV_PARENT_ID="$(query_scalar "SELECT id FROM categories WHERE parent_id IS NULL AND name_th='รายได้' LIMIT 1;")"
COGS_PARENT_ID="$(query_scalar "SELECT id FROM categories WHERE parent_id IS NULL AND name_th='ต้นทุนขาย' LIMIT 1;")"
EXP_PARENT_ID="$(query_scalar "SELECT id FROM categories WHERE parent_id IS NULL AND name_th='ค่าใช้จ่าย' LIMIT 1;")"
VAT_PARENT_ID="$(query_scalar "SELECT id FROM categories WHERE parent_id IS NULL AND name_th='ภาษีมูลค่าเพิ่ม' LIMIT 1;")"

# Children (no subquery in ON DUPLICATE KEY UPDATE to avoid error 1093)
run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('รายได้จากการขาย','Sales','income',${REV_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"
run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('ซื้อสินค้า/วัตถุดิบ','Purchases','cogs',${COGS_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"

run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('ค่าสาธารณูปโภค','Utilities','expense',${EXP_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"
run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('ค่าเช่า','Rent','expense',${EXP_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"
run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('ค่าวัสดุสำนักงาน','Office Supplies','expense',${EXP_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"
run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('ค่าเดินทาง/ขนส่ง','Transportation','expense',${EXP_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"
run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('ค่าอาหารและรับรอง','Meals/Entertainment','expense',${EXP_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"

run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('VAT ภาษีซื้อ','VAT Input','other',${VAT_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"
run_sql "INSERT INTO categories (name_th, name_en, type, parent_id) VALUES ('VAT ภาษีขาย','VAT Output','other',${VAT_PARENT_ID}) ON DUPLICATE KEY UPDATE name_en=VALUES(name_en), type=VALUES(type);"
